...
Some restrictions apply:
 - This will only work if a device has an in- AND output, because the
 Launchpad
   classes always open both directions.
 - This will not work for devices which require an initialization,
 e.g. ProMk3, etc ...
...
def main():
    ...
    print(
        "Otherwise, opening only one input or one output channel will result "
        "in an error.")
    ...
    # DEBUG STUFF
    # lp.midi.RawWriteSysEx( [ 0, 32, 41, 2, 14, 14, 1 ] )  # Pro Mk3
    # programmer's mode ON
    # lp.midi.RawWriteSysEx( [ 0, 32, 41, 2, 14, 14, 0 ] )  # Pro Mk3
    # programmer's mode OFF (Live mode)
