...
Works with all Launchpads: Mk1, Mk2, Mini Mk3, S/Mini, Pro, Pro Mk3,
XL and LaunchKey
...
def main():
    ...
    # If you have two MK3s attached, its "1" for the first and "3" for the
    # 2nd device
    ...
    # Like the Mk3, the LPX also has two MIDI instances per device; we need
    # the 2nd one.
    # If you have two LPXs attached, its "1" for the first and "3" for the
    # 2nd device
    ...
    elif launchpad.LaunchControlXL().Check():
        lp = launchpad.LaunchControlXL()
        if lp.Open(name="control xl"):
            print("Launch Control XL")
            mode = "XL"

    elif launchpad.LaunchKeyMini().Check():
        lp = launchpad.LaunchKeyMini()
        if lp.Open(name="launchkey"):
            print("LaunchKey (Mini)")
            mode = "LKM"

    elif launchpad.Dicer().Check():
        lp = launchpad.Dicer()
        if lp.Open(name="dicer"):
            print("Dicer")
            mode = "Dcr"
    ...
    # the MF64's methods are not compatible with the Launchpad ones
        ...
        lp.LedCtrlString("HENLO!", 5, direction=-1, waitms=50)
    ...
    if mode == "LKM":
        print(
            "The LaunchKey(Mini) does not (yet) support LED activation, "
            "but you")
        print("can push some buttons or rotate some knobes now...")
        print("Auto exit if first number reaches 0")
    else:
        print(
            "---\nRandom madness. Create some events. Stops after reaching 0 "
            "(first number)")
        print(
    ...
    # now quit...
    print(
        "Quitting might raise a 'Bad Pointer' error (~almost~ nothing to "
        "worry about...:).\n\n")
