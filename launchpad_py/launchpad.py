...
# This fork:
# https://github.com/J-J-B-J/launchpad
...
class Midi:
    ...
    def OpenOutput(self, midi_id):
        ...
        if self.devOut is None:
            try:
                ...
                self.devOut = midi.Output(midi_id)
            except:
                self.devOut = None
                return False
        return True
    ...
    def ReadCheck(self):
        """Check that the input is working"""
        return self.devIn.poll()

    def ReadRaw(self):
        """Read the raw USB data"""
    ...
    def RawWriteSysEx(self, lstMessage, timeStamp=0):
        ...
        # There's a bug in PyGame's (Python 3) list-type message handling,
        # so as a workaround, we'll use the string-type message instead...
        # self.devOut.write_sys_ex( timeStamp, [0xf0] + lstMessage + [0xf7]
        # ) # old Python 2

        # array.tostring() deprecated in 3.9; quickfix ahead
        try:
            self.devOut.write_sys_ex(timeStamp, array.array('B', [
                0xf0] + lstMessage + [0xf7]).tostring())
        except:
            self.devOut.write_sys_ex(timeStamp, array.array('B', [
                0xf0] + lstMessage + [0xf7]).tobytes())
    ...
    class __Midi:
        ...
        def __del__(self):
            """
            This will never be executed, because no one knows, how many
            Launchpad instances exist(ed) until we start to count them...
            """
            # midi.quit()
            pass

        @staticmethod
        def SearchDevices(name, output=True, input_=True, quiet=True):
            """
            Returns a list of devices that matches the string 'name' and has
            in- or outputs.
            """
            ret = []
            i = 0
        ...
        @staticmethod
        def SearchDevice(name, output=True, input_=True, number=0):
            """
            Returns the first device that matches the string 'name'.
            Use number argument to pick from several devices (if available)
            """
            ret = SearchDevices(name, output, input_)
...
class LaunchpadBase(object):
    """A base class for launchpad classes to inherit from."""
    ...
    def Check(self, number=0, name="Launchpad"):
        """
        Checks if a device exists, but does not open it. Does not check whether
        a device is in use or other, strange things...
        """
        self.idOut = self.midi.SearchDevice(name, True, False, number=number)
        self.idIn = self.midi.SearchDevice(name, False, True, number=number)
...
class Launchpad(LaunchpadBase):
    ...
    def Reset(self):
        """Reset the Launchpad. Turns off all LEDs"""
        self.midi.RawWrite(176, 0, 0)

    def LedCtrlRaw(self, number, red, green):
        """
        Controls a grid LED by its raw <number>; with <green/red>
        brightness: 0..3
        ...
        if number > 199:
            ...
            if number < 0 or number > 120:
                return
            # 0-120
            led = LedGetColor(red, green)
            self.midi.RawWrite(144, number, led)

    def LedCtrlXY(self, x, y, red, green):
        """
        Controls a grid LED by its coordinates <x> and <y>  with
        <green/red> brightness 0..3
        """
    ...
    def LedCtrlRawRapid(self, allLeds):
        ...
        for i in range(0, le, 2):
            self.midi.RawWrite(146, allLeds[i],
                               allLeds[i + 1] if i + 1 < le else 0)

    def LedCtrlRawRapidHome(self):
        """
        "Homes" the next LedCtrlRawRapid() call, so it will start with the
        first LED again.
        """
    ...
    def LedAllOn(self, colorcode=None):
        """
        all LEDs on
        <colorcode> is here for backwards compatibility with the newer
        "Mk2" and "Pro"
    ...
    def LedCtrlChar(self, char, red, green, offsx=0):
        """
        Sends character <char> in colors <red/green> and lateral offset
        <offsx> (-8..8)
    ...
    def LedCtrlString(self, text, red, green, direction=None, waitms=150):
        ...
        The delays were a dirty hack, but there's little to nothing one can
        do here.
        So that's how the <waitms> parameter came into play...
        NEW   12/2016: More than one char on display!!!
        ...
        if direction == self.SCROLL_LEFT:
            ...
            for n in range((len(text) + 1) * 8):
                if n <= len(text) * 8:
                    self.LedCtrlChar(
                        text[limit((n // 16) * 2, 0, len(text) - 1)], red,
                        green, 8 - n % 16)
                if n > 7:
                    self.LedCtrlChar(text[limit((((n - 8) // 16) * 2) + 1, 0,
                                                len(text) - 1)], red, green,
                                     8 - (n - 8) % 16)
                time.wait(waitms)
        elif direction == self.SCROLL_RIGHT:
            # TODO: Just a quick hack (screen is erased before scrolling
            #  begins).
            #       Characters at odd positions from the right (1, 3, 5),
            #       with pixels at the left,
            #       e.g. 'C' will have artifacts at the left (pixel repeated).
            text = " " + text + " "  # just to avoid artifacts on full width
            # characters
            ...
            for i in text:
                for n in range(
                        4):  # pseudo repetitions to compensate the timing a
                    # bit
    ...
    def ButtonStateRaw(self):
        ...
        if self.midi.ReadCheck():
            a = self.midi.ReadRaw()
            return [a[0][0][1] if a[0][0][0] == 144 else a[0][0][1] + 96,
...
def LedGetColorByName(name):
    """
    Returns an RGB colorcode by trying to find a color of a name given
    by string <name>.
...
class LaunchpadPro(LaunchpadBase):
    """
    For 3-color "Pro" Launchpads with 8x8 matrix and 4x8 left/right/top/bottom
    rows
    ...
    def LedSetMode(self, mode):
        ...
        0 -> "Ableton Live mode"  (what we need)
    ...
    def LedCtrlBpm(self, bpm):
        ...
        frequency from
        the repetive occurrence of MIDI beat clock messages (msg 248),
        24 per beat.
        No timers/events here yet, so we fake it by sending the minimal
        amount of
        messages (25 for Pro, 26 for Mk2 (not kidding) => 28, probably safe
        value) once.
        The Pro and the Mk2 support 40..240 BPM, so the maximum time we
        block everything
        is, for 40 BPM:
          [ 1 / ( 40 BPM * 24 / 60s ) ] * 28 = 1.75s    ; (acrually one
        less, 28-1)
    ...
    def LedCtrlRaw(self, number, red, green, blue=None):
        ...
        specified by
        <red>, <green> and <blue> intensities, with can each be an integer
        between 0..63.
        If <blue> is omitted, this method runs in "Classic" compatibility
        mode and the
        intensities, which were within 0..3 in that mode, are multiplied by
        21 (0..63)
        to emulate the old brightness feeling :)
        Notice that each message requires 10 bytes to be sent. For a
        faster, but
    ...
    def LedCtrlPulseByCode(self, number, colorcode=None):
        ...
        # for Mk2: [ 0, 32, 41, 2, *24*, 40, *0*, number, colorcode ] (also
    ...
    def LedCtrlFlashByCode(self, number, colorcode=None):
        ...
        The first color is the one that is already enabled, the second one
        is the
    ...
    def LedCtrlXY(self, x, y, red, green, blue=None, mode="classic"):
        """
        Controls a grid LED by its coordinates <x>, <y> and <reg>, <green>
        and <blue>
        intensity values. By default, the old and compatible "Classic" mode
        is used
        (8x8 matrix left has x=0). If <mode> is set to "pro", x=0 will
        light up the round
        buttons on the left of the Launchpad Pro (not available on other
        models).
        This method internally uses "LedCtrlRaw()". Please also notice the
        comments
        in that one.
        """
    ...
    def LedCtrlXYByCode(self, x, y, colorcode, mode="classic"):
        ...
        By default, the old and compatible "Classic" mode is used (8x8
        matrix left has x=0).
        If <mode> is set to "pro", x=0 will light up the round buttons on
        the left of the
    ...
    def LedCtrlXYByRGB(self, x, y, lstColor, mode="classic"):
        ...
        Controls a grid LED by its coordinates <x>, <y> and a list of
        colors <lstColor>.
        <lstColor> is a list of length 3, with RGB color information, [<r>,
        <g>,<b>]
        """
    ...
    def LedCtrlChar(self, char, red, green, blue=None, offsx=0):
        """
        Sends character <char> in colors <red/green/blue> and lateral
        offset <offsx> (-8..8)
        to the Launchpad. <offsy> does not have yet any function.
        If <blue> is omitted, this method runs in "Classic" compatibility
        mode and the
        old 0..3 <red/green> values are multiplied with 21, to match the
        "Pro" 0..63 range.
        """
        char = ord(char)
        char = min(char, 255)
        ...
        for i in range(81, 1, -10):
            for j in range(8):
                ...
                if i <= sum_ < i + 8:
                    if CHARTAB[char] & 0x80 >> j:
                        self.LedCtrlRaw(sum_, red, green, blue)
                    else:
                        self.LedCtrlRaw(sum_, 0, 0, 0)
            char += 1

    def LedCtrlString(self, text, red, green, blue=None, direction=None,
                      waitms=150):
        """
        Scroll <text>, with color specified by <red/green/blue>, as fast as
        we can.
        <direction> specifies: -1 to left, 0 no scroll, 1 to right
    ...
    def ButtonStateRaw(self, returnPressure=False):
        ...
        as a list [<button>, <value>], in which <button> is the raw number of
        the buttonand <value> an intensity value from 0..127.
        >0 = button pressed; 0 = button released
        Notice that this is not (directly) compatible with the original
        ButtonStateRaw() method in the "Classic" Launchpad, which only returned
        [<button>, <True/False>].
        Compatibility would require checking via "== True" and not "is True".
        Pressure events are returned if enabled via "returnPressure".
        To distinguish pressure events from buttons, a fake button code of
        "255" is used,so the list looks like [255, <value>].
        """
        if self.midi.ReadCheck():
            a = self.midi.ReadRaw()
            ...
            #  grid buttons) and
            #  "176" (Control Change, outer buttons), random (broken) SysEx
            #  messages
            #  can appear here:
            #   ('###', [[[240, 0, 32, 41], 4]])
            #   ('-->', [])
            ...
            #   ('###', [[[240, 0, 32, 41], 4]])
            #   ('-->', [])
            #  1st one is a SysEx Message (240, 0, 32, 41, 2, 16 ),
            #  with command Mode Status (45)
            #  in "Ableton Mode" (0) [would be 1 for Standalone Mode). "247"
            #  is the SysEx termination.
            #  Additionally, it's interrupted by a read failure.
            #  The 2nd one is simply cut. Notice that these are
            #  commands usually send TO the
            #  Launchpad...
            #
            # Reminder for the "pressure event issue":
            # The pressure events do not send any button codes, it's really
            # just the pressure,
            # everytime a value changes:
            #   [[[144, 55, 5, 0], 654185]]    button hit ("NoteOn with vel
            #   > 0")
            ...
            #   [[[208, 40, 0, 0], 654567]]     ...
            #   [[[208, 0, 0, 0], 654573]]      ...
            #   [[[144, 55, 0, 0], 654614]]    button released ("NoteOn with
            #   vel == 0")
            # When multiple buttons are pressed (hold), the biggest number
            # will be returned.
            #
            # Copied over from the XY method.
            # Try to avoid getting flooded with pressure events
            if not returnPressure:
                while a[0][0][0] == 208:
                    a = self.midi.ReadRaw()
                    if not a:
                        return []
    ...
    def ButtonStateXY(self, mode="classic", returnPressure=False):
        ...
        Returns the raw value of the last button change (pressed/unpressed)
        as a list
        [ <x>, <y>, <value> ], in which <x> and <y> are the buttons
        coordinates and
        <value> is the intensity from 0..127.
        >0 = button pressed; 0 = button released
        Notice that this is not (directly) compatible with the original
        ButtonStateRaw()
        method in the "Classic" Launchpad, which only returned [ <button>,
        <True/False> ].
        Compatibility would require checking via "== True" and not "is True".
        """
        if self.midi.ReadCheck():
...
class LaunchpadMk2(LaunchpadPro):
    ...
    def ButtonStateXY(self, **kwargs):
        ...
        [<x>, <y>, <value>], in which <x> and <y> are the buttons
        coordinates and
        <svalue> the intensity. Because the Mk2 does not come with full
        analog capabilities,
        unlike the "Pro", the intensity values for the "Mk2" are either 0
        or 127.
    ...
    def LedCtrlRaw(self, number, red, green, blue=None):
        ...
        and the
        intensities, which were within 0..3 in that mode, are multiplied by 21
        (0..63)
        to emulate the old brightness feeling :)
        Notice that each message requires 10 bytes to be sent. For a faster,
        but unfortunately "not-RGB" method, see "LedCtrlRawByCode()"
        Overrides "LaunchpadPro" method
        """
    ...
    def LedCtrlRawByCode(self, number, colorcode=None):
        """
        Controls a grid LED by its position <number> and a color code
        <colorcode> from the Launchpad's color palette. If <colorcode> is
        omitted, 'white' is used. This method should be ~3 times faster that
        the RGB version "LedCtrlRaw()", which uses 10 byte, system-exclusive
        MIDI messages.
        Overrides "LaunchpadPro" method
        """
    ...
    def LedCtrlPulseByCode(self, number, colorcode=None):
        """
        Same as LedCtrlRawByCode, but with a pulsing LED. Pulsing can be
        stoppped by another Note-On/Off or SysEx message.
        Overrides "LaunchpadPro" method
        """
        if number < 0 or number > 99:
            return
        ...
        # command, following an unused "0" (that's also missing in the Pro's
        # command)
    ...
    def LedCtrlFlashByCode(self, number, colorcode=None):
        ...
        # for Pro: [ 0, 32, 41, 2, *16*, *35*, number, colorcode ] (also an
        # error in the docs)
        self.midi.RawWriteSysEx([0, 32, 41, 2, 24, 35, 0, number, colorcode])

    def LedCtrlXY(self, x, y, red, green, blue=None, **kwargs):
        """
        Controls a grid LED by its coordinates <x>, <y> and <reg>, <green>
        and <blue>
        intensity values.
    ...
    def LedCtrlXYByRGB(self, x, y, lstColor, **kwargs):
        """
        New approach to color arguments.
        Controls a grid LED by its coordinates <x>, <y> and a list of
        colors <lstColor>.
    ...
    # Overrides "LaunchpadPro" method
    def LedCtrlXYByCode(self, x, y, colorcode, **kwargs):
        """
        Controls a grid LED by its coordinates <x>, <y> and its <colorcode>.
        About three times faster than the much more comfortable RGB version
        "LedCtrlXY()
        """
        if x < 0 or x > 8 or y < 0 or y > 8:
            return
    ...
    def LedCtrlPulseXYByCode(self, x, y, colorcode, **kwargs):
        """
        Pulses a grid LED by its coordinates <x>, <y> and its <colorcode>.
        Overrides "LaunchpadPro" method
        """
    ...
    def LedCtrlFlashXYByCode(self, x, y, colorcode, **kwargs):
        """
        Flashes a grid LED by its coordinates <x>, <y> and its <colorcode>.
        Overrides "LaunchpadPro" method
        """
...
class LaunchControlXL(LaunchpadBase):
    ...
    def Open(self, number=0, name="Control XL", template=1):
        ...
        By default, user template 1 is enabled. Notice that the Launch
        Control
        actually uses 0..15, but as the pad buttons are labeled 1..8 it
        probably
    ...
    def LedCtrlXY(self, x, y, red, green):
        """
        Controls a grid LED by its coordinates <x> and <y>  with <green/red>
        brightness 0..3"""
    ...
    def InputStateRaw(self):
        """
        Returns the raw value of the last button or potentiometer change as
        a list:
...
class LaunchControl(LaunchControlXL):
    ...
    def Open(self, number=0, name="Control MIDI", template=1):
        ...
        # holding down one of the template buttons and selecting the template
        # with the lowest button row 1..8 (variable here stores that as 0..7
        # for
    ...
    def LedCtrlXY(self, x, y, red, green):
        ...
        Controls a grid LED by its coordinates <x> and <y>  with
        <green/red> brightness 0..3
        Actually, this doesn't make a lot of sense as the Control only has
        one row
        of LEDs, but anyway ...
        Overrides "LaunchControlXL" method
        """
...
class LaunchKeyMini(LaunchpadBase):
    ...
    def InputStateRaw(self):
        """
        Returns the raw value of the last button, key or potentiometer
        change as a list:
        potentiometers:   <pot.number>, <value>     , 0          ]
        buttons:          <but.number>, <True/False>, <velocity> ]
        keys:             <but.number>, <True/False>, <velocity> ]
        If a button does not provide an analog value, 0 or 127 are returned
        as velocity values.
        Because of the octave settings cover the complete note range,
        the button and potentiometer
...
class Dicer(LaunchpadBase):
    ...
    def Reset(self):
        """
        reset the Dicer
        Turns off all LEDs, restores power-on state, but does not disable
        an active light show.
        """
    ...
    def ButtonStateRaw(self):
        """
        Returns an already nicely mapped and not raw value of the last
        button change as a list:
        buttons: <number>, <True/False>, <velocity> ]
        If a button does not provide an analog value, 0 or 127 are returned
        as velocity values.
        Small buttons select either 154, 155, 156 cmd for master or 157,
        158, 159 for slave.
        Button numbers (1 to 5): 60, 61 .. 64; always
        Guess it's best to return: 1..5, 11..15, 21..25 for Master and
        101..105, ... etc for slave
        Actually, as you can see, it's not "raw", but I guess those decade
        modifiers really
    ...
    def LedSetLightshow(self, device, enable):
        ...
        self.midi.RawWrite(186 if device == 0 else 189, 0,
                           40 if enable is True else 41)

    # -- NOTE: Copied from Launchpad, won't work. The Dicer actually uses:
    # -- Byte: 0b[0HHHIIII]; HHH: 3 bits hue (000=red up to 111=green) and 4
    # bits IIII as intensity.
    ...
    def LedCtrlRaw(self, number, hue, intensity):
        """
        Controls an LED by its raw <number>; with <hue> brightness: 0..7 (
        red to green)
        and <intensity> 0..15
        For LED numbers, see grid description on top of class.
        """
...
class LaunchpadMiniMk3(LaunchpadPro):
    ...
    def LedCtrlRaw(self, number, red, green, blue=None):
        ...
        faster, but
        unfortunately "not-RGB" method, see "LedCtrlRawByCode()"
        Mk3 color data extended to 7-bit but for compatibility we still
        use 6-bit values
        """
        if number < 0 or number > 99:
            return
    ...
    def Close(self):
        ...
        Otherwise Launchpad will get stuck in programmer mode
        """
        # removed for now (LEDs would light up again; should be in the
        # user's code)
...
class LaunchpadLPX(LaunchpadPro):
    ...
    def Open(self, number=0, name="AUTO"):
        """
        Opens one of the attached Launchpad MIDI devices.
        This is one of the few devices that has different names in different
        OSs:
    ...
    def LedSetLayout(self, mode):
        ...
        Sets the button layout (and codes) to the set, specified by <mode>.
        Valid options:
        00 - Session, 01 - Note Mode, 04 - Custom 1, 05 - Custom 2,
        06 - Custom 3
        07 - Custom 4, 0D - DAW Faders (available if Session enabled),
        7F - Programmer
        """
    ...
    def LedCtrlRaw(self, number, red, green, blue=None):
        ...
        but
        unfortunately "not-RGB" method, see "LedCtrlRawByCode()"
        LPX color data extended to 7-bit but for compatibility we still
        use 6-bit values
        """
        if number < 0 or number > 99:
            return
    ...
    def ButtonStateRaw(self, returnPressure=False):
        ...
        Returns the raw value of the last button change (pressed/unpressed)
        as a list
        [ <button>, <value> ], in which <button> is the raw number of the
        button and
        ...
        Compatibility would require checking via "== True" and not "is True".
        Pressure events are returned if enabled via "returnPressure".
        Unlike the Launchpad Pro, the X does indeed return the button
        number AND the
        pressure value. To provide visibility whether a button was
        pressed or is
        hold, a value of 255 is added to the button number.
        [ <button> + 255, <value> ].
        In contrast to the Pro, which only has one pressure value for all,
        the X does
            ...
            if a[0][0][0] == 144 or a[0][0][0] == 176:
                ...
                if returnPressure:
                    if a[0][0][0] == 160:
                        ...
                        # adding 255 to make it possible to distinguish
                        # "pressed" from "pressure"
...
class MidiFighter64(LaunchpadBase):
    ...
    def LedCtrlRaw(self, number, color, mode=None):
        ...
        if mode is not None and 17 < mode < 54:
            self.midi.RawWrite(147, number - 3 * 12, mode)

    def LedCtrlRawMode(self, number, mode):
        """
        Controls the mode of a grid LED by its <number> and the mode
        <mode> of the LED.
        <number> 36..99
        <mode>   18..53 for brightness, toggling and animation
    ...
    def LedCtrlChar(self, char, colorcode, offsx=0, coloroff=0):
        """
        Displays the character <char> with color of <colorcode> and lateral
        offset
        <offsx> (-8..8) on the Midi Fighter. <offsy> does not have yet any
        function.
        <coloroff> specifies the background color.
        Notice that the call to this method is not compatible to the
        Launchpad variants,
    ...
    def LedCtrlString(self, text, colorcode, coloroff=0, direction=None,
                      waitms=150):
        """
            ...
            for n in range((len(text) + 1) * 8):
                if n <= len(text) * 8:
                    self.LedCtrlChar(
                        ...
                        colorcode, 8 - n % 16, coloroff=coloroff)
                if n > 7:
                    self.LedCtrlChar(text[limit((((n - 8) // 16) * 2) + 1, 0,
                                                len(text) - 1)], colorcode,
    ...
    def ButtonStateRaw(self):
        ...
        Returns the raw value of the last button change (pressed/unpressed)
        as a list
        [ <button>, <velocity> ], in which <button> is the raw number of
        the button and
        ...
        if self.midi.ReadCheck():
            ...
            # The Midi Fighter 64 does not support velocities. For 500
            # bucks. Lol
            # What we see here are either channel 3 or 2 NoteOn/NoteOff
            # commands,
            ...
            #    146/145 -> NoteOn
            #    130/129 -> NoteOff
            #    127     -> fixed velocity (as set by the Midi Fighter
            #    utility )

            # Mhh, I guess it's about time to think about adding MIDI
            # channels, isn't it?
    ...
    def ButtonStateXY(self):
        ...
        Returns the raw value of the last button change (pressed/unpressed)
        as a list
        [ <x>, <y>, <velocity> ], in which <x>/<y> are the coordinates of
        the grid and
...
class LaunchpadProMk3(LaunchpadPro):
    ...
    def LedCtrlRaw(self, number, red, green, blue=None):
        ...
        faster, but
        unfortunately "not-RGB" method, see "LedCtrlRawByCode()"
        ProMk3 color data extended to 7-bit but for compatibility we still
        use 6-bit values
        """
        if number < 0 or number > 99:
            return
    ...
    def ButtonStateXY(self, mode="classic", returnPressure=False):
        """
        Returns the raw value of the last button change (pressed/unpressed) as
        a list
        [ <x>, <y>, <value> ], in which <x> and <y> are the buttons coordinates
        and
        <value> is the intensity from 0..127.
        >0 = button pressed; 0 = button released
        Notice that this is not (directly) compatible with the original
        ...
        if self.midi.ReadCheck():
            ...
            # 8/2020: Try to mitigate too many pressure events that a bit (
    ...
    def Reset(self):
        """(fake to) reset the Launchpad. Turns off all LEDs"""
        self.LedAllOn(0)
